; Modified NetLogo model to simulate population dynamics of red and blue turtles with predators.

globals [
  red-count            ; population of red turtles
  blue-count           ; population of blue turtles
  predator-count       ; population of predator turtles
]
...
turtles-own [
  ...
  age                  ; age of turtles
  predator?            ; flag to identify predator turtles
]

patches-own [
  visited-by-predator? ; flag to identify if a patch has been visited by a predator
]
...
to setup-experiment
  ...
  create-turtles carrying-capacity [
    ...
    set size 2
    set age 0
    set predator? false ; Initialize all turtles as non-predators
  ]

  ; Create predator turtles
  create-turtles (carrying-capacity / 5) [
    setxy random-xcor random-ycor
    set color green
    set size 3
    set predator? true ; Identify them as predators
  ]

  ask patches [ set visited-by-predator? false ] ; Initialize patches as not visited by predators
...
to go
  reproduce        ; Addressing the question of turtle reproduction and population dynamics
  grim-reaper      ; Inspecting control of population size to prevent overpopulation
  age-turtles      ; Inspecting aging of turtles
  wander-turtles   ; Addressing the question of turtle movement
  predator-interaction ; Addressing the question of predator interaction
  tick
end

; Procedure for turtle reproduction
to reproduce
  ask turtles [
    ifelse color = red [
      set fertility floor red-fertility + age * 0.1 ; increase fertility with age
      set fertility-remainder red-fertility - (floor red-fertility)
    ]
    [
      set fertility floor blue-fertility + age * 0.1 ; increase fertility with age
      set fertility-remainder blue-fertility - (floor blue-fertility)
    ]
    ifelse (random-float 100) < (100 * fertility-remainder) [
      hatch fertility + 1 [ set age 0 set predator? false ] ; reset age of offspring to 0, offspring is not a predator
    ] [
      hatch fertility [ set age 0 set predator? false ] ; reset age of offspring to 0, offspring is not a predator
    ]
  ]
end

; Procedure to age turtles
to age-turtles
  ask turtles [
    set age age + 1
  ]
...
; Procedure for turtles to move randomly
to wander-turtles
  ask turtles [
...
; Procedure for predator interaction
to predator-interaction
  ask turtles [
    if predator? [
      ; Identify nearby patches to mark as visited by a predator
      ask patch-here [
        set visited-by-predator? true
      ]
    ]
  ]
end

; Procedure to control population size by removing excess turtles
to grim-reaper
  ...
  ; Randomly select turtles for removal if population exceeds carrying capacity
  ask turtles [
